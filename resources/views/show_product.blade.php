<x-app-layout>
    ...
    <div class="mb-6 max-w-xl mx-auto md:mx-0">
    <form action="{{ route('show_product') }}" method="GET" class="flex items-center">
        <div class="relative w-full">
            ...
            <input type="text" name="search" value="{{ request('search') }}" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full ps-10 p-2.5 shadow-sm" placeholder="Cari nama produk...">
        </div>
        
        <button type="submit" class="p-2.5 ms-2 text-sm font-medium text-white bg-indigo-600 rounded-lg border border-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 shadow-sm">
            Cari
        </button>

        @if(request('search'))
            <a href="{{ route('show_product') }}" class="p-2.5 ms-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg border border-gray-300 hover:bg-gray-200">
                Reset
            </a>
        @endif
    </form>
